...
export default async function Home() {
  ...
  const approvedConcepts = concepts.filter((concept) => concept.status === 'approved').length
  const reviewConcepts = concepts.filter((concept) => concept.status === 'review').length
  const totalTopics = new Set(concepts.map((concept) => concept.topic)).size
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-4">
        <Metric icon={<Bot />} label="Topics" value={String(totalTopics)} />
        <Metric icon={<Shirt />} label="Concepts" value={String(totalConcepts)} />
        <Metric icon={<Clock3 />} label="In review" value={String(reviewConcepts)} />
        <Metric icon={<CheckCircle2 />} label="Approved" value={String(approvedConcepts)} />
      </div>
